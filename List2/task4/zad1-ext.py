...
def hybrid_sort(array, compare):
    ...
    if length <= 10:
        ...
        midpoint = math.floor(length / 2)
        return merge(hybrid_sort(array[:midpoint], compare), hybrid_sort(array[midpoint:], compare), compare)
...
def select_algorithm(arg):
    alg_types = {'insert': insertion_sort, 'merge': merge_sort, 'quick': launch_quick_sort, 'hybrid': hybrid_sort}
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("usage: python3.8 ./task1-ext.py --type insert|merge|quick|hybrid --comp '>='|'<=' --data int|str")
        sys.exit()

    try:
        n = int(input())
        arr = list(map(data_type, input().split(' ')[:]))
        if len(arr) != n:
            raise ValueError()
    except ValueError:
        print("enter integral length of list and then proper number of values")
        sys.exit()
